...
pass="";
if(flag==1){
    welcome=new JLabel("Welcome ABC");
    welcome.setFont(new Font("tunga",Font.PLAIN,17));
    welcome.setSize(270,20);
    welcome.setLocation(70,10);
}
else{
    welcome=new JLabel("Arrange the numbers");
    welcome.setFont(new Font("tunga",Font.PLAIN,17));
    welcome.setSize(270,20);
    welcome.setLocation(70,10);
}
...
add(b1);add(b2);add(b3);add(b4);add(b5);add(b6);add(b7);add(b8);add(b9); add(next);  add(welcome);
...
if(flag==1&& pass.equals("1234")){
    ...
    Process process = runtime.exec("C:\\Windows\\notepad.exe G:\\neela\\cisco\\game\\document.decrypted");
     dispose();
    }catch(Exception ex){}
    finally{
...
if(flag!=1 || flag==1 && !pass.equals("1234")){
    new TTT1();
    dispose();
}
}  
